...
public class ExportFragment extends Fragment {
    ...
    private void addTracksToPlaylist(final String id, final String playlistId, List<DanceSong> danceSongs, final SpotifyService service) {
        ...
        api.getManyRecordings(danceSongs, Arrays.asList("spotify"), new Consumer<LinkedHashMap<DanceSong, List<DanceRecording>>>() {
            @Override
            public void accept(LinkedHashMap<DanceSong, List<DanceRecording>> map) {
                if (map.size() == 0) {
                    Toast.makeText(ExportFragment.this.getActivity(), R.string.no_spotify_ids, Toast.LENGTH_LONG).show();
                } else {
                    ...
                    service.addTracksToPlaylist(id, playlistId, null, body, new Callback<Pager<PlaylistTrack>>() {
                        ...
                        public void success(Pager<PlaylistTrack> playlistTrackPager, Response response) {
                            Toast.makeText(ExportFragment.this.getActivity(), String.format(getString(R.string.playlist_was_created), title), Toast.LENGTH_LONG).show();
                        }
    ...
    public class ExportPlaylistToYoutubeTask extends AsyncTask<List<DanceSong>, String, String> {
        ...
        protected String doInBackground(List<DanceSong>... params) {
            ...
            if (token == null) {
                return getString(R.string.unable_to_obtain_token);
            }
            ...
            try {
                YouTube.Playlists.Insert playlistInsertCommand = youtube.playlists().insert("snippet", createPlaylist(playlistName, String.format(getString(R.string.playlist_generated_by), getString(R.string.app_name))));
            ...
            try {
                api.getManyRecordingsSync(params[0],  Arrays.asList("youtube"), new Consumer<LinkedHashMap<DanceSong, List<DanceRecording>>>() {
                    @Override
                    public void accept(LinkedHashMap<DanceSong, List<DanceRecording>> map) {
                        if (map.size() == 0) {
                            ...
                            results.add(String.format(getString(R.string.playlist_was_created), title));
                        }
                    }
                });
            } catch (IOException e) {
